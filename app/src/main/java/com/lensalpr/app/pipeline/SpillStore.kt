...
class SpillStore(private val dir: File, private val maxFiles: Int = 600, private val maxBytes: Long = 400L * 1024 * 1024) {
    ...
    fun adopt(): Int {
        val images = runCatching { dir.apply { mkdirs() }.listFiles { file -> file.name.endsWith(".jpg") } }
            .getOrNull()
            .orEmpty()
        val highest = images.mapNotNull { file ->
    ...
    /** Synchronised against [offer]: the writer and the drain loop share this directory. */
    @Synchronized
    fun poll(): SpilledCrop? {
        val images = dir.listFiles { file -> file.name.endsWith(".jpg") } ?: return null
        val newest = images.maxByOrNull { it.name } ?: return null
        val index = newest.name.removePrefix("spill_").removeSuffix(".jpg").toIntOrNull() ?: -1
        val metaFile = File(dir, newest.name.removeSuffix(".jpg") + ".json")
